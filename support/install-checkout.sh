...
commands=(
  ...
  agent-hook-pre-edit
  agent-hook-pre-mcp
  agent-hook-pre-search
  agent-hook-pre-search-muse
  agent-hook-prompt-submit
  agent-hook-session-end
  agent-hook-session-end-claude
  ...
  for ((index = count - 1; index >= 0; index--)); do
    ...
    if [[ "${prior_states[$index]}" == symlink ]]; then
      if ! ln -sfn -- "${prior_targets[$index]}" "$target"; then
        printf 'agentguard: rollback failed to restore symlink: %s\n' "$target" >&2
      fi
    elif ! rm -f "$target"; then
      printf 'agentguard: rollback failed to remove new link: %s\n' "$target" >&2
    fi
  done
...
printf 'installed agentguard commands to %s\n' "$BIN_DIR"
printf 'installed agentguard manpages to %s\n' "$MAN_DIR"
